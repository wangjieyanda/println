...
func _read(fname string, c *Config) (*Config, error) {
	...
	var exportValueMap = map[string]map[string]*ExportValue{}
	for k, v := range c.data {
		var exportValue = map[string]*ExportValue{}
		for k1, v1 := range v {
			if _, ok := exportValue[k1]; !ok {
				exportValue[k1] = &ExportValue{}
			}
			exportValue[k1].Position = v1.position
			exportValue[k1].V = v1.v

		}
		exportValueMap[k] = exportValue
	}
	c.CacheData = exportValueMap
